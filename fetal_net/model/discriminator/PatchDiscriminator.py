...
def d_layer(layer_input, filters, conv, strides=(2,2,2), f_size=4, normalization=True):
    """Discriminator layer"""
...
def build_discriminator(input_shape, df=64, conv=None, depth=4, only_xy_pool=2):
    if conv is None:
    ...
    for i in range(0, depth):
        pool_size = (2, 2, 1) if i < only_xy_pool else (2, 2, 2)
...
def build_discriminator_3d(input_shape, df=32, depth=4, only_xy_layers=2, **kwargs):
    return build_discriminator(input_shape, df=df, conv=Conv3D, depth=4, only_xy_pool=2)