...
def input2discriminator(A_patches, B_patches, d_out_shape):
    ...
    d_y_batch[A_patches.shape[0]:, ...] = 1 - d_y_batch[A_patches.shape[0]:, ...]
...
def input2gan(patchesA, segsA, patchesB, d_out_shape):
    ...
    g_y_batch = [
        segsA,
        np.clip(np.random.uniform(0.05, 0.15, size=[patchesA.shape[0]] + list(d_out_shape)[1:]), a_min=0, a_max=1),
        np.clip(np.random.uniform(0.85, 0.95, size=[patchesB.shape[0]] + list(d_out_shape)[1:]), a_min=0, a_max=1)
    ]
...
def main(overwrite=False):
    ...
    seg_A = Activation('linear', name='s')(decoder_model(emb_A))
    # seg
    ...
    # dis
    valid_A = Activation('linear', name='dA')(frozen_dis_model(emb_A))
    valid_B = Activation('linear', name='dB')(frozen_dis_model(emb_B))
    ...
    for epoch in range(config["n_epochs"]):
        ...
        with tqdm(range(n_train_steps // config["gen_steps"]), dynamic_ncols=True,
            ...
            if gen_metrics[0] < best_loss:
                ...
                print('Saving Model...')
                with open(os.path.join(config["base_dir"], "seg_{}_{:.3f}.json".format(epoch, gen_metrics[0])), 'w') as f:
                    f.write(seg_model.to_json())
                seg_model.save_weights(os.path.join(config["base_dir"], "seg_{}_{:.3f}.h5".format(epoch, gen_metrics[0])))
