...
from keras.layers import Activation, Conv2D, Flatten, Dense, LeakyReLU, Conv3D
...
if not "gdS_loss_ratio" in config:
    config["gdS_loss_ratio"] = 0.5
if not "gdE_loss_ratio" in config:
    config["gdE_loss_ratio"] = 0.5
...
def main(overwrite=False):
    ...
    #
    from fetal_net.model.unet.ae3D import ae_model_3d
    encoder_model, decoder_model = ae_model_3d(input_shape=config["input_shape_seg"],
                                               dropout_rate=config['dropout_rate'],
                                               xy_pool_layers=2)
    ...
    def dis():
        ...
        conv = LeakyReLU()(Conv3D(128, 1)(inputs))
        conv = LeakyReLU()(Conv3D(128, 4, strides=2, padding='same')(conv))
        conv = LeakyReLU()(Conv3D(128, 4, strides=2, padding='same')(conv))
        conv = LeakyReLU()(Conv3D(256, 2, strides=1, padding='valid')(conv))
        flat = Flatten()(conv)
    ...
    seg_dis_model = PatchDiscriminator.build_discriminator_3d(input_shape=seg_model.output_shape[1:])
