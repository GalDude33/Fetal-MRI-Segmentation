...
def input2discriminator(real_patches, real_segs, fake_segs, d_out_shape, mul_merge=True):
    if mul_merge:
        real = mul_merge_maps(real_patches, add_noise_to_segs(real_segs))
        fake = mul_merge_maps(real_patches, fake_segs)
    else:
...
def main(overwrite=False):
    ...
    combined_model.compile(loss=['binary_crossentropy', seg_loss_func],
